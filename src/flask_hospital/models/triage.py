...
class Triage(db.Model):
    ...
    admission_id: Mapped[int] = mapped_column(db.Integer, db.ForeignKey("admission.id"), nullable=False, unique=True)
    priority_level_id: Mapped[int] = mapped_column(db.Integer, db.ForeignKey("priority_level.id"), nullable=True)
    ...
    priority_level: Mapped["PriorityLevel"] = relationship("PriorityLevel", back_populates="triage", lazy="joined")
